...
// =========================================================================
// 1. 🏆 홈 상단 기획전 배너 (로켓프레시 & 로켓와우 순환 728x90 & 광고주 모집)
// =========================================================================
interface TopMainSlimBannerProps {
  onOpenAdPartnership?: () => void;
}

export const TopMainSlimBanner: React.FC<TopMainSlimBannerProps> = ({ onOpenAdPartnership }) => {
  const [isVisible, setIsVisible] = useState(true);
  const [activeTab, setActiveTab] = useState<0 | 1 | 2>(0); // 0: 로켓프레시, 1: 로켓와우, 2: 광고주 모집
  const [imageErrorMap, setImageErrorMap] = useState<Record<string, boolean>>({});

  useEffect(() => {
    recordBannerImpression('TOP_MAIN');
    // 8초마다 로켓프레시 -> 로켓와우 -> 광고주 모집 자동 순환
  ...
  return (
    <div className="w-full bg-white/95 border-b border-gray-200/90 shadow-2xs relative z-10 font-sans backdrop-blur-xs py-1 sm:py-1.5 px-3">
      <div className="max-w-4xl mx-auto relative flex flex-col items-center justify-center">
        ...
        {/* 배너 탭 인디케이터 (전환 탭 & 광고주 입점 모집) */}
        <div className="flex items-center gap-1.5 mb-1 select-none flex-wrap justify-center">
          <button
            type="button"
          ...
          <button
            ...
            className={`text-[10px] font-black px-2.5 py-0.5 rounded-full transition-all cursor-pointer flex items-center gap-1 ${
              activeTab === 2
                ? 'bg-indigo-600 text-white shadow-2xs scale-105 ring-2 ring-indigo-300'
                : 'bg-indigo-50 text-indigo-700 hover:bg-indigo-100 border border-indigo-200 shadow-2xs'
            }`}
          >
            <span>📢 여기 광고하실 분 (입점 문의)</span>
          </button>
        </div>

        {/* 🎯 메인 배너 카드 (탭 2는 광고주 모집 배너, 탭 0/1은 쿠팡 배너) */}
        {activeTab === 2 ? (
          <div className="relative inline-block w-full max-w-[728px] group">
            <div
              onClick={onOpenAdPartnership}
              className="w-full bg-gradient-to-r from-indigo-950 via-slate-900 to-indigo-900 text-white rounded-lg p-2 sm:p-2.5 shadow-xs border border-indigo-400/40 flex items-center justify-between gap-2.5 cursor-pointer hover:border-amber-400 transition-all"
            >
              <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-amber-400 text-indigo-950 flex items-center justify-center font-black text-sm flex-shrink-0 shadow-xs">
                  📢
                </div>
                <div className="min-w-0">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <span className="px-1.5 py-0.5 bg-amber-400 text-indigo-950 text-[9px] font-black rounded">
                      광고주 모십니다
                    </span>
                  ...
                  <p className="text-[10px] sm:text-[11px] text-indigo-200 truncate mt-0.5">
                    전체 자영업자 사장님 대상 상단 독점 노출 · 식자재/포장/POS/세무 B2B 최적
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-1 flex-shrink-0">
                <button
                  ...
                  className="px-2.5 py-1 sm:py-1.5 bg-amber-400 hover:bg-amber-300 text-indigo-950 rounded-lg text-[10px] sm:text-xs font-black flex items-center gap-0.5 shadow-xs transition active:scale-95 cursor-pointer whitespace-nowrap"
                >
                  <span>상담 신청</span>
                ...
                <button
                  ...
                  className="w-5 h-5 rounded-full bg-black/40 hover:bg-black/75 text-white/80 hover:text-white flex items-center justify-center transition-all cursor-pointer ml-0.5"
                  title="배너 닫기"
                >
        ...
        {/* ⚖️ 공정거래위원회 필수 표기 문구 & 광고 제휴 링크 */}
        <div className="text-[9px] sm:text-[10px] text-gray-500 font-medium text-center mt-1 select-none flex items-center justify-center gap-2 flex-wrap">
          {activeTab !== 2 ? (
            <span>※ {COUPANG_FTC_DISCLOSURE}</span>
          ) : (
            <span className="text-indigo-600 font-bold">
              ※ 트레이드미 공식 B2B 광고 제휴 안내 · 24시간 실시간 입점 상담
            </span>
          )}
          <button
            type="button"
            onClick={onOpenAdPartnership}
            className="text-[10px] font-black text-indigo-700 hover:text-indigo-900 underline cursor-pointer inline-flex items-center gap-0.5"
          >
            <span>[광고 제휴 문의]</span>
          </button>
        </div>
...
// =========================================================================
// 5. 📢 [우리 동네 가맹점 모아보기] 피드형 광고주 제휴 모집 카드
// =========================================================================
export const StoreFeedAdPartnershipCard: React.FC<{ onOpenAdPartnership?: () => void }> = ({
  onOpenAdPartnership,
}) => {
  const [isVisible, setIsVisible] = useState(true);
  if (!isVisible) return null;
  ...
  return (
    <div className="relative rounded-2xl sm:rounded-3xl bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 text-white border-2 border-indigo-400/50 shadow-md p-3.5 sm:p-4 overflow-hidden transition-all hover:border-amber-400 group">
      {/* 우측 상단 닫기 X 버튼 */}
      <button
        type="button"
        onClick={(e) => {
        ...
        className="absolute top-2.5 right-2.5 w-6 h-6 rounded-full bg-white/10 hover:bg-white/25 text-white/80 hover:text-white flex items-center justify-center transition-all z-20 cursor-pointer"
        title="닫기"
      >
        <X className="w-3.5 h-3.5 pointer-events-none" />
      </button>
      ...
      >
        <div className="flex items-start gap-3 min-w-0 pr-6 sm:pr-0">
          <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-2xl bg-amber-400 text-slate-950 flex items-center justify-center font-black text-xl flex-shrink-0 shadow-sm mt-0.5 sm:mt-0 group-hover:scale-105 transition-transform">
            📢
          </div>
          <div className="space-y-0.5 min-w-0">
            ...
            <p className="text-xs text-slate-300 font-medium leading-relaxed line-clamp-1">
              식자재 도매 · 포장용기 · POS/키오스크 · 주방설비 · 세무 등 지역 사장님들께 직접 노출됩니다.
            </p>
          </div>
        </div>

        <div className="flex items-center gap-2 self-end sm:self-center flex-shrink-0 pt-0.5 sm:pt-0">
          <button
            type="button"
            onClick={(e) => {
              e.stopPropagation();
            ...
            className="px-3.5 py-1.5 sm:py-2 bg-gradient-to-r from-amber-400 to-amber-500 hover:from-amber-300 hover:to-amber-400 text-slate-950 font-black text-xs rounded-xl shadow-md transition active:scale-95 flex items-center gap-1 cursor-pointer whitespace-nowrap"
          >
            <span>광고 입점 문의</span>
...
// =========================================================================
// 6. 🎟️ [내 교환권 보관함] 공식 스폰서십 광고주 제휴 모집 카드
// =========================================================================
export const WalletAdPartnershipCard: React.FC<{ onOpenAdPartnership?: () => void }> = ({
  onOpenAdPartnership,
}) => {
  const [isVisible, setIsVisible] = useState(true);
  if (!isVisible) return null;
  ...
  return (
    <div className="relative rounded-2xl bg-gradient-to-r from-purple-950 via-indigo-950 to-slate-900 text-white border-2 border-purple-400/50 shadow-md p-3 sm:p-3.5 overflow-hidden transition-all hover:border-amber-400 group">
      {/* 우측 상단 닫기 X 버튼 */}
      <button
        type="button"
        onClick={(e) => {
        ...
        className="absolute top-2 right-2 w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-white/10 hover:bg-white/25 text-white/80 hover:text-white flex items-center justify-center transition-all z-20 cursor-pointer"
        title="닫기"
      >
        <X className="w-3 h-3 sm:w-3.5 sm:h-3.5 pointer-events-none" />
      </button>
      ...
      >
        <div className="flex items-center gap-2.5 sm:gap-3 min-w-0 pr-5 sm:pr-0">
          <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-amber-400 text-purple-950 flex items-center justify-center font-black text-lg flex-shrink-0 shadow-sm group-hover:scale-105 transition-transform">
            🎟️
          </div>
          <div className="space-y-0.5 min-w-0">
            <div className="flex items-center gap-1.5 flex-wrap">
              <span className="px-1.5 py-0.5 bg-purple-400 text-purple-950 text-[9px] font-black rounded-md">
                스폰서십 제휴
              </span>
            ...
            <p className="text-[10px] sm:text-[11px] text-purple-200 truncate">
              거래와 정산이 가장 활발한 사장님들의 필수 공간! 보관함 전용 브랜드 배너 제휴
            </p>
          </div>
        </div>
        ...
        <button
          ...
          className="px-2.5 sm:px-3 py-1.5 bg-amber-400 hover:bg-amber-300 text-slate-950 font-black text-xs rounded-xl shadow-xs transition active:scale-95 flex items-center gap-0.5 flex-shrink-0 whitespace-nowrap cursor-pointer"
        >
          <span>광고 문의</span>
          <ChevronRight className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
        </button>
