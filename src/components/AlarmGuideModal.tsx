...
import { X, Bell, BellRing, Smartphone, ShieldCheck, AlertCircle } from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 z-[170] bg-black/70 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in">
      <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl flex flex-col overflow-hidden border border-orange-100 animate-in zoom-in-95 duration-200">
        ...
        {/* Body Content */}
        <div className="p-5 space-y-3.5 text-xs text-gray-700 leading-relaxed max-h-[75vh] overflow-y-auto font-sans">
          ...
          <div className="flex items-start gap-3 p-3 bg-amber-50/90 border border-amber-200 rounded-2xl">
            ...
            <div>
              ...
              <p className="text-[11px] text-amber-900 leading-normal">
                아래 버튼을 누르면 스마트폰 상단이나 브라우저에 <strong>&apos;알림을 허용하시겠습니까?&apos;</strong> 창이 뜹니다. 반드시 <strong className="underline decoration-amber-600 font-extrabold">&apos;허용&apos;</strong>을 눌러주셔야 소리와 푸시 알림을 정상적으로 받으실 수 있습니다.
              </p>
            </div>
          </div>
          ...
          <div className="flex items-start gap-3 p-3 bg-gray-50 border border-gray-200/80 rounded-2xl">
            ...
            <div>
              <h4 className="font-bold text-gray-900 text-xs mb-0.5">
                💡 아이폰(iOS Safari) 이용자 안내
              </h4>
              <p className="text-[11px] text-gray-600 leading-normal">
                아이폰 Safari에서는 화면 하단 공유 버튼(네모+화살표)을 눌러 <strong>[홈 화면에 추가]</strong>를 하시면, 앱처럼 화면이 꺼져 있어도 실시간 푸시를 안정적으로 받으실 수 있습니다.
              </p>
            </div>
          </div>
          ...
          {/* Warning if blocked */}
          {isPermissionDenied && (
            <div className="flex items-start gap-2.5 p-3 bg-rose-50 border border-rose-200 rounded-2xl text-rose-900">
              <AlertCircle className="w-4 h-4 text-rose-600 flex-shrink-0 mt-0.5" />
              <div className="text-[11px]">
                <strong>현재 브라우저에서 알림이 [차단]되어 있습니다.</strong>
                <p className="text-rose-700 mt-0.5">
                  주소창 왼쪽 🔒(자물쇠) 또는 설정 아이콘을 눌러 &apos;알림&apos; 권한을 &apos;허용&apos;으로 변경해 주세요.
                </p>
              </div>
            </div>
          )}
